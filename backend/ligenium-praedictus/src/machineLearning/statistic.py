...
def get_quartils(numbers: List[Number]) -> Tuple[float, float]:
...
def get_whiskers(numbers: List[Number]) -> Tuple[float, float]:
...
def get_outliers_by_indicies(numbers: List[Number]) -> Tuple[List[int], List[int]]:
    if(len(numbers) < 3):
        return ([], [])
    values = numbers.copy()
